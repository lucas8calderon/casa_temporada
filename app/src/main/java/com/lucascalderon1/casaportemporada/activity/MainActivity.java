...
import com.lucascalderon1.casaportemporada.activity.autenticacao.LoginActivity;
...
public class MainActivity extends AppCompatActivity implements AdapterAnuncios.OnClick {
    ...
    private void recuperaAnunciosFiltros() {
        DatabaseReference reference = FirebaseHelper.getDatabaseReference()
                .child("anuncios_publicos");
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
    ...
    private void showDialogLogin() {
        ...
        builder.setMessage("Você não esta logado no aplicativo, deseja logar agora?");
        ...
        if (resultCode == RESULT_OK) {
            if (requestCode == REQUEST_FILTRO) {
                ...
                if (filtro.getQtdQuarto() > 0 || filtro.getQtdBanheiro() > 0 || filtro.getQtdGaragem() > 0) {
                    //recupera os anuncios dos filtros
                    recuperaAnunciosFiltros();
                }
